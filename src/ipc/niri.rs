...
use crate::state::{AppState, WorkspaceInfo, WindowState, WorkspaceState};
...
#[serde(untagged)]
enum NiriResponse {
    Ok { #[serde(rename = "Ok")] data: ResponseData },
    Err { #[serde(rename = "Err")] err: String },
}
    ...
    loop {
        ...
        match reader.read_line(&mut line).await {
            ...
            Ok(_) => {
                ...
                if !trimmed.is_empty() && !trimmed.contains("Ok") && !trimmed.contains("WindowLayouts") {
...
async fn handle_event(line: &str, state: &Arc<RwLock<AppState>>) -> Result<()> {
    let message: NiriMessage = serde_json::from_str(line)
        .context("Failed to parse niri message")?;
                ...
                NiriResponse::Ok { data } => {
                    match data {
                        ResponseData::Workspaces(workspaces) => {
                            ...
                            app_state.workspaces = workspaces.iter().map(|ws| WorkspaceInfo {
                                id: ws.id,
                                idx: ws.idx,
                                name: ws.name.clone(),
                            }).collect();
                            ...
                            if let Some(active_ws) = workspaces.iter().find(|ws| ws.idx.is_some()) {
                                ...
                                eprintln!("[INIT_WS] {}/{} (id={}, name={:?})", active_ws.idx.unwrap_or(1), total, active_ws.id, active_ws.name);
                            }
                        }
        ...
        NiriMessage::Event(event) => match event {
            NiriEvent::WorkspaceActivated { data } => {
                ...
                if let Some(ws) = app_state.workspaces.iter().find(|w| w.id == data.id) {
                    ...
                    eprintln!("[WS_SWITCH✓] {} → {}/{} (name={:?})", data.id, idx, total, name);
                } else {
                    eprintln!("[WS_SWITCH✗] id {} NOT in stored list (have {} ws)", data.id, app_state.workspaces.len());
                }
            }
            ...
            NiriEvent::WorkspacesChanged { workspaces } => {
                ...
                if let Some(active_ws) = workspaces.iter().find(|ws| ws.is_focused || ws.is_active) {
                    ...
                    eprintln!("[WS_LIST_CHANGED] {}/{} (id={}, active={}, focused={})", active_ws.idx.unwrap_or(1), total, active_ws.id, active_ws.is_active, active_ws.is_focused);
                }
            }
